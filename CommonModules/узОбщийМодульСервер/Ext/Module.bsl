...
Процедура ОтправитьПисьмо(ДопПараметры) Экспорт
	...
	Попытка
		...
		Конецесли;
	Исключение
		ДокОбъект.Важность = Перечисления.ВариантыВажностиВзаимодействия.Обычная;
	КонецПопытки;
	...
	//agroeco+ miv 11.12.2018
	//ДокОбъект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовЭлектронныхПисем.ПростойТекст");
	ДокОбъект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовЭлектронныхПисем.HTML");
	
	ФорматированныйТекст = Новый ФорматированныйДокумент;
	ШрифтЖирный = Новый Шрифт(,14,Истина,Истина,Истина);
	
	МассивСообщений = СтрРазделить(ТекстПисьма, Символы.ПС);
	
	ТекстХТМЛ = "<html <head></head>";
	ТекстХТМЛ = ТекстХТМЛ + "<body>";
	ТекстХТМЛ  = ТекстХТМЛ + "<br><b>" + МассивСообщений[0] + "</b></br>";
	Для Инд = 1 По МассивСообщений.Количество() - 1 Цикл
		ТекстХТМЛ  = ТекстХТМЛ + "<p>" + МассивСообщений[Инд] + "</p>";
	КонецЦикла;
	ТекстХТМЛ  = ТекстХТМЛ + "<p></p>";
	
	ТекстХТМЛ  = ТекстХТМЛ + "<a href=""http://app1c0/C1_DEV_TASK_0/ru/?C=ОткрытьЗадачу," + ДопПараметры.Ссылка.УникальныйИдентификатор() + """>" + ДопПараметры.Ссылка.Наименование + "</a>";
	
	ТекстХТМЛ = ТекстХТМЛ + "</body></html>";

	ДокОбъект.ТекстHTML = ТекстХТМЛ;  
