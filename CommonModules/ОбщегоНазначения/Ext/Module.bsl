...
Процедура лПауза(Сек) Экспорт	
    scr = Новый COMОбъект("WScript.Shell");
    scr.Run("ping 127.0.0.1 -n "+Формат(Сек+1, "ЧГ=0"),0,1);
КонецПроцедуры

Процедура ВызватьОжиданиеНаБлокировке() Экспорт
	
	НачатьТранзакцию();
	
	//Блокировка = Новый БлокировкаДанных;
	//СтрокаБлокировки = Блокировка.Добавить("Константа.ДатаПроверкиВерсии");
	//СтрокаБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	//Блокировка.Заблокировать();
	
	// для мониторинга включения технологического журнала
	Константы.ДатаПроверкиВерсии.Установить(Константы.ДатаПроверкиВерсии.Получить());
...
Процедура ЗапросКЗаблокированномуОбъекту() Экспорт
	
	//Запрос = Новый Запрос;
	//Запрос.Текст = 
	//"ВЫБРАТЬ
	//|	ДатаПроверкиВерсии.Значение
	//|ИЗ
	//|	Константа.ДатаПроверкиВерсии КАК ДатаПроверкиВерсии";
	...
	Для к = 1 По 10 Цикл
		...
		Попытка
			...
			//Запрос.Выполнить()
		Исключение
			ЗаписьЖурналаРегистрации("ПроверкаЧтенияТехнологическогоЖурнала",,,,"Тестовая ошибка. " + ОписаниеОшибки());
...
Процедура ВызватьОшибку() Экспорт 
	...
	// да, это специально вызывается чтобы произошла ошибка 
	Справочники.НетТакогоСправочника.СоздатьЭлемент();
	
КонецПроцедуры

Функция НужноВыполнитьПервоначальнуюНастройку() Экспорт 
	
	//Возврат НЕ Константы.ПервоначальначНастройкаВыполнена.Получить();
	Возврат Ложь;
	
КонецФункции

Процедура ВыполнитьПервоначальнуюНастройку(СообщенияОбОшибках) Экспорт 
	
	НаборЗаписей = РегистрыСведений.Настройки.СоздатьНаборЗаписей();
	Запись = НаборЗаписей.Добавить();
	
	Запись.Период = ТекущаяДата();
	
	// каталог конфигурационного файла
	СписокПутейКонф = ВыполнитьПоискПутейКаталогаКонф();
	Если СписокПутейКонф.Количество() > 0 Тогда
		Запись.ПутьККонфигурационномуФайлу = СписокПутейКонф[0];
	Иначе
		СообщенияОбОшибках = СообщенияОбОшибках + Символы.ПС + "Не удалось определить каталог размещения конфигурационного файла";
	КонецЕсли;
	...
	Иначе
		СообщенияОбОшибках = СообщенияОбОшибках + Символы.ПС + "Не удалось определить каталог размещения файлов технологического журнала";
	КонецЕсли;

	Запись.ПериодХраненияЛоговТехнологическогоЖурнала = 1;
	Запись.КоличествоПотоковДляОбработкиЛогов = 1;
	Запись.ТипСУБД = Перечисления.ТипСУБД.SQLServer;
	
	// версия
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Версия = Лев(СистемнаяИнформация.ВерсияПриложения,3);
	Если Версия = "8.3" Тогда
		Запись.ВерсияПлатформы1С = Перечисления.ВерсияПлатформы.v83;
	Иначе
		Запись.ВерсияПлатформы1С = Перечисления.ВерсияПлатформы.v82;
	КонецЕсли;
	
	Запись.ДлительностьЗапросовПорог1 = 8;
	Запись.МетодОбработкиПороговДлительности = Перечисления.ОбработкаПорговДлительности.Первый;
	Запись.ВключатьПланыЗапросов = Истина;
	
	Запись.WSDL_УдаленноеПодключение = "https://skynet.gilev.ru/QueryTJ/ws/QueryTJ.1cws?wsdl";
	Запись.WSDL_УдаленноеПодключениеРезерв = "http://kitt.gilev.ru:8888/querytj/ws/QueryTJ.1cws?wsdl";
	Запись.WSDL_Пользователь = "ws";
	Запись.WSDL_Пароль = "ws";
	
	Запись.WSDL_Status_УдаленноеПодключение = "https://info.gilev.ru/status/ws/status.1cws?wsdl";
	Запись.WSDL_Status_УдаленноеПодключениеРезерв = "	http://otto.gilev.ru:8080/status/ws/status.1cws?wsdl";
	Запись.WSDL_Status_Пользователь = "ws";
	Запись.WSDL_Status_Пароль = "ws";
	
	НаборЗаписей.Записать();
	
	Константы.ПервоначальначНастройкаВыполнена.Установить(Истина);
	
КонецПроцедуры

Функция ВыполнитьПоискПутейКаталогаКонф()
...
Функция ПериодТекущихнастроек() Экспорт 
	...
	Возврат ПериодТекущихНастроек;
	
КонецФункции

Функция ПолучитьСостояниеТестирования() Экспорт 
	
	Результат = Новый Структура("Этап, Успех, СообщениеОбОшибке",0);
	
	НаборЗаписей = РегистрыСведений.СостояниеТестирования.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
