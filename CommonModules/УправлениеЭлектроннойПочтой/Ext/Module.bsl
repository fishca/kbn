...
Процедура ПолучитьПисьма(ДанныеУчетнойЗаписи, ЕстьОшибки, Получено, МассивСозданныхПисем)
	...
	Если ИдЗагружать.Количество() <> 0 Тогда
		...
		ОтветственныйЗаОбработкуПисем = Справочники.Пользователи.ПустаяСсылка(); //agroeco+ ПолучитьОтветственногоЗаОбработкуПисьмаПоУмолчанию(ДанныеУчетнойЗаписи.Ссылка);
...
Функция ЗаписатьВходящееЭлектронноеПисьмо(
	...
	//agroeco+ miv Формируем структуру взаимодействий
	аэОбщийМодульСервер.ЗаписатьСтрутуруВзаимодействия(Письмо.Ссылка);
