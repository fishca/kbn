...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	Наименование = ?(ПустаяСтрока(Данные.Наименование), НСтр("ru = 'Без описания'"), "#" + Формат(Данные.Ссылка.Код, "ЧГ=0") + " " + Данные.Наименование);
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Или ВнешнееСоединение Тогда
	Дата = Формат(Данные.Ссылка.ДатаСоздания, ?(ПолучитьФункциональнуюОпцию("ИспользоватьДатуИВремяВСрокахЗадач"), "ДЛФ=DT", "ДЛФ=D"));
	Представление = Метаданные.НайтиПоТипу(ТипЗнч(Данные.Ссылка)).Представление();
#Иначе	
	Дата = Формат(Данные.Ссылка.ДатаСоздания, "ДЛФ=D");
	Представление = НСтр("ru = 'Задача'");
#КонецЕсли
